...
# 给外骨骼机器人添加障碍物
# 规划运行部分已注释，可配合exo_stand_fk进行目标关节角度的运动规划
...
class MoveItObstaclesDemo:
    def __init__(self):
        ...
        # 初始化ROS节点
        rospy.init_node('exo_stand_obstacles')
        ...
        left = MoveGroupCommander('left_group')
        right = MoveGroupCommander('right_group')

        
        # 获取终端link的名称
        left_end_effector_link = left.get_end_effector_link()
        right_end_effector_link = right.get_end_effector_link()
        
        # 设置位置(单位：米)和姿态（单位：弧度）的允许误差
        left.set_goal_position_tolerance(0.01)
        left.set_goal_orientation_tolerance(0.05)
        right.set_goal_position_tolerance(0.01)
        right.set_goal_orientation_tolerance(0.05)
       
        # 当运动规划失败后，允许重新规划
        left.allow_replanning(True)
        right.allow_replanning(True)
        
        # 设置目标位置所使用的参考坐标系
        reference_frame = 'base_link'
        left.set_pose_reference_frame(reference_frame)
        right.set_pose_reference_frame(reference_frame)
        
        # 设置每次运动规划的时间限制：5s
        left.set_planning_time(5)
        right.set_planning_time(5)
        ...
        # # 控制机械臂运动到目标位置
        # left.set_pose_target(target_pose, left_end_effector_link)
        # left.go()
        # right.set_pose_target(target_pose, right_end_effector_link)
        # right.go()
        ...
        # # 控制机械臂运动到目标位置
        # left.set_pose_target(target_pose2, left_end_effector_link)
        # left.go()
        # right.set_pose_target(target_pose2, right_end_effector_link)
        # right.go()
